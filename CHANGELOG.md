...
Stopping the stack (and the "Removing old containers" step of a Coolify redeploy) took ~3 minutes. `docker events` during a redeploy showed Coolify stops the old containers **strictly serially** (~30s apart), and the picture workers plus `api` each sat out their **full ~30s** stop timeout before `SIGKILL` because they never exit on `SIGTERM`. Serial stopping makes every timeout fully additive: `4 workers × 30s + api 30s ≈ 150s` accounted for essentially the whole window. The other services (`auth`, `db`, `reverseproxy`, `website`, `backup`, `keycloak-export`) already exit in a few seconds.

- **`stop_grace_period` added to slow / safe-to-kill services** — `keycloak-export` (`1s`), the `background-worker` anchor (`2s`, inherited by all four workers), `website` (`2s`), and `api` (`5s`). Picture jobs live in a DB-backed queue so a mid-picture kill is safe (the row stays pending and is reprocessed on restart); the export sidecar and stateless website have nothing to flush; and `reverseproxy` (the API's only ingress) is stopped long before `api` in the serial teardown, so there are no in-flight external requests for gunicorn to drain. `db`, `auth`, `reverseproxy`, and `backup` keep the default grace period since they already stop quickly.
- **Coolify's "Stop Grace Period" (30s default) overrides compose `stop_grace_period`.** Redeploy `docker events` captures confirmed the old containers ignored the per-service compose values and died at exactly the Coolify UI grace ceiling (30s, then 10s after it was lowered). Lowering the Coolify setting to **10s** cut the "Removing old containers" step from ~3min to ~60s. That setting is global (applies to every container), so it can't safely go much lower — `db` (postgres) wants a few seconds for a clean fast-shutdown, though it's crash-safe via WAL even if killed.
- **`stop_signal: SIGKILL` on the `background-worker` anchor and `api`.** These are the only services that ignore `SIGTERM` — they were observed dying at the grace ceiling on every redeploy (uniform across all four workers), i.e. always `SIGKILL`ed after a pointless wait. Sending `SIGKILL` up front makes them die instantly and, unlike `stop_grace_period`, `stop_signal` is *not* overridden by Coolify's grace setting — so it works under Coolify and is independent of the grace value. Safe because workers are DB-queue-backed (jobs resume) and `api` has no in-flight ingress by the time it's stopped (reverseproxy dies first; DB transactions roll back; orphaned temp uploads are handled by the existing orphan-prune).
